...
void GL_initLEDC()
{
    ...
    ledc_channel_config_t ledc_channel[3] = {
        ...
        for (ch = 0; ch < 3; ch++) {
            ...
            ledc_set_duty_and_update(LEDC_LOW_SPEED_MODE,g_Channels[ch],g_RGBValues[ch],0xFFFF);
        }
...
extern void GL_setRGB(uint8_t *RGBLevels)
{
	...
	if (g_ledcstatus)
	{
		for (ch = 0; ch < 3; ch++) {
		        ...
		        ledc_set_duty_and_update(LEDC_LOW_SPEED_MODE,g_Channels[ch],RGBLevels[ch],0xFFFF);
		}
	}
}
